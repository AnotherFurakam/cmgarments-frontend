  ...
  const getByDate = async () => {
    ...
    if (dateStart === "" || dateEnd === "") {
      ...
      productState.setProducts(
        await productService.filterByDateProduct(dateStart, dateEnd)
      );
    }
  };

  return (
    <div className="d-flex gap-2 border-2 border p-2 rounded-4">
      ...
      <FilterButton onClick={getByDate}>
        <BsSearch color="#fff" size={20} />
      </FilterButton>
      <FilterButton onClick={handleReset}>
        <FiRefreshCcw color="#fff" size={20} />
      </FilterButton>
    </div>
  );
};
